...
export default function useRelatorios() {
  ...
  const carregarRelatorios = async () => {
    try {
      // Buscar Vendas
      const resVendas = await fetch(`http://127.0.0.1:5000/relatorio/vendas?id_restaurante=${idRestaurante}`);
      const jsonVendas = await resVendas.json();
      if (resVendas.ok) setDadosVendas(jsonVendas);

      // Buscar Itens Populares
      const resItens = await fetch(`http://127.0.0.1:5000/relatorio/itens-populares?id_restaurante=${idRestaurante}`);
